		...
		for (int i = 0; i < dNotes.size(); i++) {
				...
				if (dNotes.get(i).equalsIgnoreCase(mdr.getDNotes())) {
					if (dType.get(i).equalsIgnoreCase("文件")) {
						if (mdr.getDName().endsWith("_FILE")) {
							// 文件类型，没有变化
							newColums.add(ChineseUtil.getPinYinHeadChar(dNotes
									.get(i))
									+ "_FILE");
							newColums.add(ChineseUtil.getPinYinHeadChar(dNotes
									.get(i))
						...
						if (mdr.getDName().endsWith("_FILE")) {
							...
							if (dType.get(i).equalsIgnoreCase(mdr.getDType())) {
								if (dType.get(i).equalsIgnoreCase("字符串") && (mdr
										.getDLength().compareTo(dLength.get(i))!=0)) {
									mCol = true;
								}
							} else {
								mCol = true;
							...
							if (mCol) {
								...
								sbColumn.append(" comment '").append(
										dNotes.get(i)).append("'");
							}
						}
					}
			...
			if (!existColumn) {
				...
				if(otherToFile || dType.get(i).equalsIgnoreCase("文件")){
					...
					sbColumn.append(ChineseUtil.getPinYinHeadChar(" add "+dNotes.get(i))+"_FILE ")
						.append(" ");
				}
				else{
