 ...
 * 		formName  表单ID
 */
Form.prototype.initFormSubmit = function(formName) {
	// 表单提交设置
 ...
 * @param {} $_list_list  List对象
 */
Form.prototype.initReset = function() {
	$('* [id=resetBtn]').bind('click', this.clearForm);
};
...
Form.prototype.modify = function(url, callback) {
	...
	if(!self.list.canEdit()) return;
	this.clearForm();
	this.setEditState("update");// 修改状态为更新
	self.list.showHiddenEdit({id : chkValue, editState : this.getEditState()});
